/*	Cuben::LeastSq static object
	...
	Derived from Chapter 5 of Timothy Sauer's 'Numerical Amalysis'
*/
	...
	namespace DiffInt {
		...
		float intfdx_adaptTrap(float(*f)(float), float x0, float x1, float tol) {
			...
			if (error < 3 * tol) {
				...
				result = intfdx_adaptTrap(f, x0, xMid, 0.5f * tol) + intfdx_adaptTrap(f, xMid, x1, 0.5f * tol);
			}
			return result;
		}		
		...
		float intfdx_adaptSimp(float(*f)(float), float x0, float x1, float tol) {
			...
			if (error < 15 * tol) {
				...
				result = intfdx_adaptSimp(f, x0, xMid, 0.5f * tol) + intfdx_adaptSimp(f, xMid, x1, 0.5f * tol);
			}
			return result;
		}
		...
		bool test() {
			std::cout << "int{f dx} [degree 2] = " << intfdx_gaussQuad(f, -1.0f, 1.0f, 2) << std::endl;
			std::cout << "int{f dx} [degree 3] = " << intfdx_gaussQuad(f, -1.0f, 1.0f, 3) << std::endl;
			std::cout << "int{f dx} [degree 4] = " << intfdx_gaussQuad(f, -1.0f, 1.0f, 4) << std::endl;
			return true;
		}
	}
}
