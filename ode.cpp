...
Eigen::VectorXf cuben::ode::euler(float(*dxdt)(float, float), Eigen::VectorXf ti, float x0) {
...
Eigen::VectorXf cuben::ode::trap(float(*dxdt)(float, float), Eigen::VectorXf ti, float x0) {
...
Eigen::VectorXf cuben::ode::taylor2nd(float(*dxdt)(float, float), float(*d2xdt2)(float, float), float(*d2xdtdx)(float, float), Eigen::VectorXf ti, float x0) {
    int n = ti.rows();
    float dt = 0.0f;
    Eigen::VectorXf xi(n);
    for (int i = 0; i < n; i++) {
...
Eigen::VectorXf cuben::ode::rk4(float(*dxdt)(float, float), Eigen::VectorXf ti, float x0) {
    // Runge-Kutta, order 4
    int n = ti.rows();
    float dt = 0.0f;
    float s1 = 0.0f;
...
Eigen::MatrixXf cuben::ode::eulerSys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::VectorXf ti, Eigen::VectorXf x0) {
    int nSteps = ti.rows();
    int nDims = x0.rows();
    float dt;
...
Eigen::MatrixXf cuben::ode::trapSys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::VectorXf ti, Eigen::VectorXf x0) {
    int nSteps = ti.rows();
    int nDims = x0.rows();
    float dt;
...
Eigen::MatrixXf cuben::ode::midSys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::VectorXf ti, Eigen::VectorXf x0) {
    int nSteps = ti.rows();
    int nDims = x0.rows();
    float h = 1.0f;
...
Eigen::MatrixXf cuben::ode::rk4Sys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::VectorXf ti, Eigen::VectorXf x0) {
    int nSteps = ti.rows();
    int nDims = x0.rows();
    float h = 1.0f;
...
void cuben::ode::rk23Sys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::Vector2f tInt, Eigen::VectorXf x0, Eigen::VectorXf& ti, Eigen::MatrixXf& xi) {
    int nDims = x0.rows();
    int n = 1;
    int order = 3;
    float relErr = 0.0f;
    float absErr = 0.0f;
    float minStepSize = 16.0f * cuben::fundamentals::relEps(tInt(1));
    ...
    while (ti(n - 1) < tInt(1)) {
        ...
        if (relErr < cuben::constants::relDiffEqTol) {
            // Update state with o3, compute h for next step
            ti = cuben::fundamentals::safeResize(ti, n + 1);
            xi = cuben::fundamentals::safeResize(xi, n + 1, nDims);
            ti(n) = ti(n - 1) + h;
            xi.row(n) = o3;
            maxStepSize = std::min(tInt(1) - ti(n), 10.0f * h);
            h = std::max(minStepSize, std::min(maxStepSize, (0.8f * std::pow(cuben::constants::relDiffEqTol / relErr, 1.0f / (order + 1.0f)))));
            minStepSize = 16.0f * cuben::fundamentals::relEps(ti(n));
            n++;
        } else {
            // Try again with half step size
            h *= 0.5f;
            if (h < minStepSize) { throw cuben::exceptions::xBelowMinStepSize(); }
        }
    }
}

void cuben::ode::bs23Sys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::Vector2f tInt, Eigen::VectorXf x0, Eigen::VectorXf& ti, Eigen::MatrixXf& xi) {
    int nDims = x0.rows();
    int n = 1;
    int order = 3;
    float relErr = 0.0f;
...
void cuben::ode::rk45Sys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::Vector2f tInt, Eigen::VectorXf x0, Eigen::VectorXf& ti, Eigen::MatrixXf& xi) {
...
void cuben::ode::dp45Sys(void(*dxdt)(float, Eigen::VectorXf, Eigen::VectorXf&), Eigen::Vector2f tInt, Eigen::VectorXf x0, Eigen::VectorXf& ti, Eigen::MatrixXf& xi) {
    int nDims = x0.rows();
    int n = 1;
    int order = 5;
    float relErr = 0.0f;
...
Eigen::VectorXf cuben::ode::impEuler(float(*fInd)(float, float), float(*fImp)(float, float), Eigen::VectorXf ti, float x0) {
    int n = ti.rows();
    float dt = 0.0f;
    float ox = 0.0f;
    float dx = 0.0f;
    ...
    for (int i = 0; i < n; i++) {
        if (i == 0) {
            ...
            } while (nn < cuben::constants::iterLimit && std::abs(xi(i) - ox) > cuben::constants::iterTol);
        }
    }
    return xi;
}

Eigen::VectorXf cuben::ode::impTrap(float(*fInd)(float, float), float(*fImp)(float, float), Eigen::VectorXf ti, float x0) {
    int n = ti.rows();
    float dt = 0.0f;
    float ox = 0.0f;
    float dx = 0.0f;
...
Eigen::VectorXf cuben::ode::modab2s(float(*dxdt)(float, float), Eigen::VectorXf ti, float x0) {
    int n = ti.rows();
    float dt = 0.0f;
    Eigen::VectorXf xi(n);
    for (int i = 0; i < n; i++) {
...
Eigen::VectorXf cuben::ode::modam2s(float(*fInd)(float, float), float(*fImp)(float, float), Eigen::VectorXf ti, float x0) {
    int n = ti.rows();
    float dt = 0.0f;
    float ox = 0.0f;
    float dx = 0.0f;
...
Eigen::VectorXf cuben::ode::modms2s(float(*fInd)(float, float), float(*fImp)(float, float), Eigen::VectorXf ti, float x0) {
...
Eigen::VectorXf cuben::ode::modam3s(float(*fInd)(float, float), float(*fImp)(float, float), Eigen::VectorXf ti, float x0) {
...
Eigen::VectorXf cuben::ode::modam4s(float(*fInd)(float, float), float(*fImp)(float, float), Eigen::VectorXf ti, float x0) {
