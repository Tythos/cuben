...
cuben::Alfg::Alfg(unsigned int nj, unsigned int nk) {
    j = nj;
    k = nk;
    stateVector = initialize(j, k);
}

Eigen::VectorXf cuben::Alfg::initialize(unsigned int j, unsigned int k) {
    Eigen::VectorXi piDigits(100); piDigits <<
    ...
    Eigen::VectorXi eDigits(100); eDigits <<
