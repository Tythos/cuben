/*	Cuben::Pde static object
	Partial diffferential equation algorithms
	Derived from Chapter 8 of Timothy Sauer's 'Numerical Amalysis'
*/
...
namespace Cuben {
	namespace Pde {
		Eigen::MatrixXf expParaEuler(Eigen::VectorXf coeffs, float(&f)(float,float,float), float(&ul)(float), float(&ur)(float), float(&u0)(float), Eigen::Vector2f xBounds, Eigen::Vector2f tBounds, float dx, float dt) {
			Eigen::VectorXf xi = Cuben::Fund::initRangeVec(xBounds(0), dx, xBounds(1));
...
//			epsilonm = coeffs(2) / (dt * dt);
		...
		Eigen::MatrixXf finDiffElliptic(Eigen::VectorXf coeffs, float(&f)(float,float), float(&uxbcLower)(float), float(&uxbcUpper)(float), float(&uybcLower)(float), float(&uybcUpper)(float), Eigen::Vector2f xBounds, Eigen::Vector2f yBounds, float dx, float dy) {
			Eigen::VectorXf xi = Cuben::Fund::initRangeVec(xBounds(0), dx, xBounds(1));
			Eigen::VectorXf yi = Cuben::Fund::initRangeVec(yBounds(0), dy, yBounds(1));
			int nx = xi.rows();
			int ny = yi.rows();
			int nInt = (nx - 2) * (ny - 2);
				...
				for (int j = 1; j < ny - 1; j++) {
					...
					for (int k = 0; k < 9; k++) {
						...
						if (!isBoundaryFilter[k]) {
							...
							linNdcs(k) = Cuben::Fund::sub2ind(intDims, subNdx);
						}
					}
			...
			for (int i = 1; i < nx - 1; i++) {
				for (int j = 1; j < ny - 1; j++) {
					...
					int linNdx = Cuben::Fund::sub2ind(intDims, subNdx);
					uij(i,j) = uVec(linNdx);
				}
			}
			return uij;
		...
		bool test() {
			Eigen::VectorXf coeffs(6); coeffs << 1.0f,0.0f,1.0f,0.0f,0.0f,0.0f;
			Eigen::Vector2f xBounds; xBounds << 0.0f,1.0f;
			Eigen::Vector2f yBounds; yBounds << 1.0f,2.0f;
			Eigen::MatrixXf uij = finDiffElliptic(coeffs, zeroForce, bc1, bc2, bc3, bc4, xBounds, yBounds, 0.25f, 0.25f);
			for (int i = 0; i < uij.rows(); i++) {
				std::cout << uij.col(i) << std::endl << std::endl;
			}
			return true;
		}
	}
